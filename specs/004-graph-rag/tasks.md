...
- [X] T053 核验 Java 调用图在评测语料中是否产生内部调用边：4 条 Java 结构性查询 graph_edge_path_summary 全空、SC-001 收益全部来自 DDL，需确认 US1 收益或记录原因 per US1/FR-023 (partial)
  - **核验结论**: Java 调用图提取器（graph/extractors/java_call_graph.py，有单元测试）工作正常；但 001 评测 Java 语料（com.example.service.UserService）无内部方法间调用——findById/getActiveUsers 仅调用字段方法 repository.xxx（外部类型），validateToken 仅抛异常，故 `graph_edge` 表中 Java scope（351193748123680768）无 calls/called_by 边。SC-001 结构性收益全部来自 DDL FK 边（真实外键关系）。此为语料数据局限而非代码缺陷；丰富 Java 语料（含类内方法调用）属后续评测语料扩展，不在本收敛任务范围。
