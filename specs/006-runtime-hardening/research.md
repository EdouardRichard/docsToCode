...
| **MRR** | **不下降**（单侧非回归：下降 >1% 判回归；上升记录为环境敏感漂移，见 §0.3 注） | 逐查询单侧非回归 |
| **nDCG@K** | **不下降**（单侧非回归，同上） | 逐查询单侧非回归 |
...
1. **冒烟一致（单侧非回归）**：001 基线 11 条 Markdown/Java 评测集在 writer 与 reader 两实例形态各重跑一遍，非延迟指标与 `baseline_report.json` 逐条对照、按非回归下界判定——下降超过 1% 即判回归，高于基线记录为环境敏感漂移（延迟与成本标注环境敏感，详见下方注）。
...
> **1% 容差判定语义（单侧非回归，T086 固化）**：条目 1 的"1% 相对容差"按**非回归下界**应用——任一非延迟指标相对基线**下降超过 1%** 即判定回归；**高于基线**（重索引导致的语料漂移）被记录为环境敏感漂移，不构成检索行为改变、不作质量声明（FR-027/SC-009）。eval/instance_form_smoke.py 的 no_regression（单侧）为该判定口径的实现，within_tolerance（双侧）仅作为可重复性记录字段保留。
