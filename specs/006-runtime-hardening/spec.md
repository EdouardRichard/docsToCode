...
- **FR-028**: 作为替代义务，006 的通过判定为三项全过（澄清 Q3 固化）：（1）001 基线 11 条 Markdown/Java 评测集在 writer 与 reader 两实例形态各重跑一遍，非延迟指标与 `baseline_report.json` 逐条对照、按非回归下界判定——任一非延迟指标相对基线下降超过 1% 即判定回归；高于基线的漂移记录为环境敏感、不作质量声明（延迟与成本标注环境敏感）；（2）001–005 既有 pytest 验收测试集全部通过；（3）宪法硬约束在 writer + reader 部署形态上成立（FR-023~FR-025）。回归冒烟不构成质量对照。
...
- **SC-009**: 非回归三项判定全过（澄清 Q3 固化）：（1）001 基线 11 条评测集在 writer 与 reader 两实例形态重跑的非延迟指标与 `baseline_report.json` 逐条对照、按非回归下界判定——任一非延迟指标相对基线下降超过 1% 即判定回归，高于基线的漂移记录为环境敏感、不作质量声明；（2）001–005 既有验收测试集全部通过；（3）硬性指标保持（与 SC-008 一致）。不设质量阈值、不作质量声明（对照评测要求：无）。
