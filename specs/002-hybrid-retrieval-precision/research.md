...
| **MRR (mean)** | 0.9091 | **可度量提升** | 原 11 条子集相对**同会话 Dense 基线**严格正增量（相对口径，2026-09-04 修订；旧绝对阈值 ≥ 0.95 为旧环境水位操作化，已废止，见 §0.6 末注） | SC-001 / FR-021 |
| **nDCG@K (mean)** | 0.9329 | **可度量提升** | 原 11 条子集相对同会话 Dense 基线严格正增量（相对口径，2026-09-04 修订；旧绝对阈值 ≥ 0.96 已废止，见 §0.6 末注） | SC-001 / FR-021 |
...
> **阈值口径修订（2026-09-04，用户决策选 B）**：上述绝对阈值（MRR ≥ 0.95 / nDCG ≥ 0.96）为旧环境
> （单机原始语料）下的水位操作化。共享评测环境与重索引（`reindex_eval_qdrant.py`）后，同会话 Dense
> 基线臂的绝对水位整体下移（2026-09-04 实测原 11 条 Dense MRR 0.7576），绝对阈值在不同环境间不可迁移。
> 宪法原则 X 的判定语义是"在固定评测集上证明可度量收益"，与绝对水位无关。**现行口径**：原 11 条子集上
> MRR/nDCG 相对同会话 Dense 基线**严格正增量** + Recall 非降 + 实测硬性指标全过（泄漏=0 / Schema=100% /
> 定位=100%）。绝对水位仅在报告 `original_subset_gate` 中作参考记录，不作门禁。判定实现见
> `eval/run_comparison.py`，报告契约 `eval-comparison-report.schema.json`。
